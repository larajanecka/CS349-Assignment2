...
function createModelModule() {
    ...
    var ImageModel = function(
        ...
        if (!(
                ...
                && (_.isNumber(parseInt(rating)) && rating >= 0 && rating <= 5)
            ))
        {
    ...
    _.extend(ImageModel.prototype, {
        ...
        removeListener: function(listener_fn) {
            this.listeners = _.filter(this.listeners, function(list) {
                return !_.isEqual(list,listener_fn);
            });
        },
        ...
        setCaption: function(caption) {
            if(caption && !_.isString(caption)) {
                throw new Error("Caption must be a string");
            }
            var self = this;
            self.modificationDate = new Date();
            this.caption = caption;

            _.each(this.listeners, function(listener) {
                listener(self, new Date());
            });
        },
        ...
        setRating: function(rating) {
            ...
            if(!_.isUndefined(rating) && (isNaN(rating) || rating < 0 || rating > 5)) {
                throw new Error("Invalid rating, rating must be a number in range 0-5");
            }
            this.rating = rating;
        ...
        getModificationDate: function() {
            return (this.modificationDate) ? this.modificationDate : new Date();
        }
    });
    ...
    var ImageCollectionModel = function() {
        ...
        this.imageFunc = _.bind(this.imageUpdate, this);
    };
    ...
    _.extend(ImageCollectionModel.prototype, {
        ...
        addImageModel: function(imageModel) {
            imageModel.addListener(this.imageFunc);
            this.imageModels.push(imageModel);

            _.each(this.listeners, function(listener) {
                listener(IMAGE_ADDED_TO_COLLECTION_EVENT, this, imageModel, new Date())
            });
        },
        ...
        removeImageModel: function(imageModel) {
            imageModel.removeListener(this.imageFunc);
            this.imageModels = _.filter(this.imageModels, function(model) {
                return !_.isEqual(model, imageModel)
            });


            _.each(this.listeners, function(listener) {
                listener(IMAGE_REMOVED_FROM_COLLECTION_EVENT, this, imageModel, new Date())
            });
        },
        ...
        imageUpdate: function(imageModel, eventTime) {
            _.each(this.listeners, function(listener) {
                listener(IMAGE_META_DATA_CHANGED_EVENT, this, imageModel, new Date())
            });
        }
